...
const format = (res) => {
    ...
    try {
        rows.forEach((row) => {
            ...
            item.city = row.dimensions[0];
            item.user = row.metrics[0].values[0];
            item.userRemark = round(row.metrics[0].values[0] / ttfCondition.values[0] * 100, 2);
            item.newUser = row.metrics[0].values[1];
            item.newUserRemark = round(row.metrics[0].values[1] / ttfCondition.values[1] * 100, 2);
            item.session = row.metrics[0].values[2];
            item.sessionRemark = round(row.metrics[0].values[2] / ttfCondition.values[2] * 100, 2);
            item.bounceRate = round(row.metrics[0].values[3], 2);
            item.pvpSession = round(row.metrics[0].values[4], 2);
            item.avgSession = tohmsString(row.metrics[0].values[5]);
            item.goal1Rate = round(row.metrics[0].values[6], 2);
            item.goal1Comp = row.metrics[0].values[7];
            item.goal1CompRemark = round(row.metrics[0].values[7] / ttfCondition.values[7] * 100, 2);
            item.goal1Value = row.metrics[0].values[8];
            item.goal1ValueRemark = round(row.metrics[0].values[8] / ttfCondition.values[8] * 100, 2);
