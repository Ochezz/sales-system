<template>
    <div class="row visit-reservation-detail-wrap">
      <ReserveManageRegistModal></ReserveManageRegistModal>
      <ReserveManageInfoModal :datas="INFO_DATA"></ReserveManageInfoModal>
      <h4>사전방문 예약 관리 현황</h4>
      ...
      <div class="top-wrap">
        ...
        <div class="customer-search-wrap">
          ...
          <div class="btn-wrap">
            ...
            <button class="btn-control setting-btn"><router-link :to="{path:'/reservation/pre/visit'}">예약현황</router-link></button>
          </div>
        </div>
      </div>
      <ve-table 
...
<script>
  import ReserveManageRegistModal from '../Modal/ReserveManageRegistModal.vue'
  import ReserveManageInfoModal from '../Modal/ReserveManageInfoModal.vue'
  ...
  export default {
    components: {
      ReserveManageRegistModal,
      ReserveManageInfoModal
    },
    data () {
      return {
        nowDate: '',
    ...
    methods: {
      ...
      async infoShow (rowDatas) {
        ...
        try {
          await VISIT.settingList(reqParam).then(res => {
