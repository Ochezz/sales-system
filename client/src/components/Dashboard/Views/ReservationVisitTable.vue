<template>
    <div class="row visit-reservation-detail-wrap">
      <VisitResevationInfoModal :datas="INFO_DATA"></VisitResevationInfoModal>
      <h4>사전방문 예약 현황</h4>
      ...
      <div class="top-wrap">
        ...
        <div>
          ...
          <div class="btn-wrap">
            ...
            <button class="btn-control setting-btn"><router-link :to="{path:'/reservation/pre/manage'}">예약관리현황</router-link></button>
          </div>
        </div>
      </div>
      <ve-table 
...
<script>
  import VisitResevationInfoModal from '../Modal/VisitResevationInfoModal.vue'

  import * as VISIT from '../../../api/visit'
  ...
  export default {
    ...
    computed: {
      ...
      columns () {
        let columns = this.tableData.length !== 0 ? [
          ...
          this.tableData[0].visitNum !== undefined ? { field: 'visitNum', key: 'visitNum', title: '방문인원', width: 120, align: 'center' } : {},
    ...
    methods: {
      ...
      async infoShow (rowDatas) {
        ...
        try {
          await VISIT.list(reqParam).then(res => {
      ...
      async excelDown () {
        ...
        this.loadingInstance.show()
        try {
          await VISIT.csvDownload(reqParam)
        }
        catch (e) {
          console.error(e)
          await this.loadingInstance.close()
            ...
            if (res.data.length === 0) {
              ...
              res.data.forEach((element) => {
                this.TABLE_DATA.push({
                  ...
                  name: element.name,
                  phone: element.phone,
                  visitNum: '본인(' + element.visitNum + '인)',
